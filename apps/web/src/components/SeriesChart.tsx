...
export const SeriesChart = memo(function SeriesChart({ series, samples }: { series: Series; samples: Sample[] }) {
  ...
  const { lines, logDomain, logTicks } = useMemo(() => {
    ...
    const lines = groups.map(([runId, pts], i) => {
      const data = pts.map((s) => ({ iteration: s.iteration, y: mode === 'value' ? s.value : Math.max(s.errorAbs, LOG_FLOOR) }));
      if (mode === 'error') for (const d of data) (min = Math.min(min, d.y)), (max = Math.max(max, d.y));
      return {
        runId,
    ...
    if (mode !== 'error' || !lines.length) return { lines, logDomain: undefined, logTicks: undefined };
      ...
      <div className="graph-paper relative h-[220px]">
        {empty ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart margin={{ top: 12, right: 16, bottom: 4, left: 4 }}>
              <XAxis
                type="number"
                dataKey="iteration"
              ...
              <Tooltip
                ...
                formatter={(v: number) => (mode === 'value' ? fmt.value(v) : fmt.errorAbs(v))}
